...
> **Ход врага целиком** (1 свайп на доске + каскады + навыки): **`GAMEPLAY.md`**. Этот файл — только
> выбор и применение **навыков** после фазы доски.

## Ход противника (кратко)

1. Статусы и подготовка (`EndTurnExecutor.prepareEnemyBeforeActions`).
2. Если `makeEnemyMove: true` — **один** случайный легальный свайп (`GameScreen.makeMove`), каскады,
   начисление очков врагу.
3. `enemyActions()` — **все** перки с `show && enable`, перебор рук и перков сверху вниз (не один
   «лучший»).
4. Конец хода → передача герою.

При `makeEnemyMove: false` шаг 2 пропускается.

---

## Текущая реализация (навыки)
