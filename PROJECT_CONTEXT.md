...
> **Полное описание механики хода (1 совмещение за ход, каскады, перки, порт в веб):**
> **`GAMEPLAY.md`**

1. **Ход героя**:
    - Игрок делает **одно успешное** совмещение на доске (неудачные свайпы откатываются; после
      первого матча доска блокируется до следующего хода)
    - **GameBoardAdapter.handleMatches()** → каскады (цепочки совпадений в рамках того же хода)
    - **UpdateStockExecutor** → обновляет ресурсы по совпавшим гемам
    - **MatchPerkExecutor** → применяет правила совпадения (HeroRules)
    - Игрок тратит очки на **один или несколько** навыков (ограничения — `category`, ресурсы)
    - **PerkExecutor.execute()** → применяет навыки и их эффекты
    - **EndTurnExecutor.execute()** → завершает ход героя (кнопка «Закончить ход»)

2. **Ход противника**:
    - **EndTurnExecutor.startEnemyTurn()** → начинает ход противника
    - Если `makeEnemyMove: true` → противник делает **один** свайп на поле, затем каскады
    - Противник автоматически применяет **все** доступные навыки (сверху вниз по рукам)
