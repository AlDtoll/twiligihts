...
**Важно:** за один ход героя — **одно успешное совмещение** на доске (каскады входят в него), затем
навыки. Это не бесконечный match-3. Подробно: **`GAMEPLAY.md`**.
...
**`Settings.types` (число цветов гемов на доске):** не ставить меньше **4**. При трёх цветах генерация
начальной доски без готовых совпадений долго перебирает варианты → заметный лаг старта сцены. Число
типов на поле должно совпадать с наборами в `gemSettings` и стоками по этим `gemType` (и обычно —
рукой, чтобы ресурс можно было тратить).
...
| Файл                             | Содержание                                             |
|----------------------------------|--------------------------------------------------------|
| `GAMEPLAY.md`                    | **Игровой цикл:** 1 совмещение за ход, каскады, перки, враг — читать перед портом в веб |
...
| `ENEMY_AI.md`                    | Поведение противника (перки, порядок, `makeEnemyMove`) |
